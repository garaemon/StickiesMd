...
struct ContentView: View {
  ...
  var body: some View {
    ...
    .onDrop(of: [.image], isTargeted: nil) { providers in
      ...
      provider.loadFileRepresentation(forTypeIdentifier: UTType.image.identifier) { url, error in
        ...
        DispatchQueue.main.async {
          ...
          let tempCopy = FileManager.default.temporaryDirectory.appendingPathComponent(
    ...
    .contextMenu {
      ...
      Menu("Font Color") {
        Button {
          viewModel.updateFontColor("#000000")
        } label: {
          Label("Black", systemImage: "circle.fill").foregroundColor(.black)
        }
        Button {
          viewModel.updateFontColor("#333333")
        } label: {
          Label("Dark Gray", systemImage: "circle.fill").foregroundColor(Color(white: 0.2))
        }
        Button {
          viewModel.updateFontColor("#666666")
        } label: {
          Label("Gray", systemImage: "circle.fill").foregroundColor(Color(white: 0.4))
        }
        Button {
          viewModel.updateFontColor("#0000FF")
        } label: {
          Label("Blue", systemImage: "circle.fill").foregroundColor(.blue)
        }
        Button {
          viewModel.updateFontColor("#FF0000")
        } label: {
          Label("Red", systemImage: "circle.fill").foregroundColor(.red)
        }
      }
...
#Preview {
  ContentView(
    viewModel: StickyNoteViewModel(note: StickyNote(fileURL: URL(fileURLWithPath: "/tmp/test.org")))
  )
}
