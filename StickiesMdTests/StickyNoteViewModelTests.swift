...
final class StickyNoteViewModelTests: XCTestCase {
    ...
    override func setUp() {
        super.setUp()
        tempFileURL = FileManager.default.temporaryDirectory.appendingPathComponent("test_\(UUID().uuidString).md")
    ...
    func testUpdateFontColor() {
        let expectation = expectation(forNotification: .stickyNoteFontColorChanged, object: nil) { notification in
    ...
    func testToggleLineNumbers() {
        ...
        let expectation = expectation(forNotification: .stickyNoteLineNumbersChanged, object: nil) { notification in
            let updatedNote = notification.object as? StickyNote
